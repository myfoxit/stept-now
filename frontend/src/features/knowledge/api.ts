...
export type SourceType = 'files' | 'urls' | 'text'
...
export const knowledgeApi = {
  ...
  createSource: (body: { type: SourceType; name: string; config?: Record<string, unknown> }) =>
    api.post<Source>(ws('/knowledge/sources'), { config: {}, ...body }),
  updateSource: (id: string, body: { name?: string; config?: Record<string, unknown> }) =>
  ...
  // --- search --------------------------------------------------------------
  search: (body: { query: string; k?: number; source_ids?: string[] | null }) =>
